...
void LoadFromFileDlist(DList** Dhead, DList** Dtail) {
    ...
    current->next = NULL;
    *Dtail = current;
    cout << "Файл упешно сохранен." << endl;
    fclose(data);
}
...
void Split(Node** head, Node** begin) {
    if (head == NULL or begin == NULL)
        cout << "Один из этих списков не существует!" << endl;
    else {
        Node* tail = (*head);
