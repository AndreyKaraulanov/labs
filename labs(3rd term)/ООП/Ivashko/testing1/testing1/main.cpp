...
int string_counter(string name) {
    int count = 0;
    fin.open(name);
    string key;
    while (!fin.eof()) {
...
string string_from(string string1) {
    string res;
    fin.open(string1);
    string key, temp;
    while (!fin.eof()) {
...
string string_into(string string1) {
    string res;
    fin.open(string1);
    string temp, key;
    while (!fin.eof()) {
...
void create_objects_array(Change data[], int count_words, string source, string processed) {
    int count = 0;
...
string change_word(string source_string, int number_word, Change* data) {
    ...
    for (int i = 0; i < source_string.length();) {
        for (int j = 0; j < number_word; j++) {
            for (int source = 0, l = i; source < data[j].get_size_word_for_change(); source++, l++) {
                if (source_string[l] == data[j].word_for_change[source]) {
                    int true_cnt = 0;
                    if (source_string.length() - i >= data[j].get_size_word_for_change()) {
                        for (int t = 0, x = i; t < data[j].get_size_word_for_change(); t++, x++)
                        {
                            if (source_string[x] == data[j].word_for_change[t]) true_cnt++;
                        }
                    }
                    is = false;
    ...
    step:
        if (!is) {
            ...
            processed_string += data[count].word_to_change;
            i = i + data[count].get_size_word_for_change();
        }
    }
